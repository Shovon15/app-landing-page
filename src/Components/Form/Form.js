...
const Form = () => {
	...
	useEffect(() => {
		...
		axios
			...
			.then((res) => {
				if (res.status === 200) {
					const distArr = [];
					res?.data?.data?.map((data) => distArr.push({ value: data.id, label: data.name }));
					setDistrict(distArr);
				}
			})
			.catch((error) => {
	...
	function getThana(id) {
		get(`${rootUrl}thana-list/${id}`)
			.then((res) => {
				const thanaArr = [];
				res?.data?.data?.map((data) => thanaArr.push({ value: data.id, label: data.name }));
	...
	const handleFormSubmit = (e) => {
		e.preventDefault();
		showSuccessToast("Demo registration complete!");
					...
					<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-2">
						<div className="flex flex-col text-start px-10">
							...
							<input
								...
								value={values?.name}
							/>
							{!isValided && <p className="text-red-500">{errors?.name?.[0]}</p>}
						</div>
						<div className="flex flex-col text-start px-10">
							...
							/>
							{!isValided && <p className="text-red-500">{errors?.business_name?.[0]}</p>}
						</div>
						<div className="flex flex-col text-start px-10">
							<label className="required">
								Email
							...
							<input
								...
								value={values?.email}
							/>
							{!isValided && <p className="text-red-500">{errors?.email?.[0]}</p>}
						</div>
						<div className="flex flex-col text-start px-10">
							<label className="required">
								Phone number
							...
							/>
							{!isValided && <p className="text-red-500">{errors?.phone_number?.[0]}</p>}
						</div>
						...
						<div className="flex flex-col text-start px-10">
							...
							<Select
								...
								styles={customStyles}
							/>
							{!isValided && <p className="text-red-500">{errors?.district_id?.[0]}</p>}
						</div>
						{/* ----------------------thana------------------ */}
						<div className="flex flex-col text-start px-10">
							<label>Thana</label>
						...
						<div className="flex flex-col text-start px-10">
							...
							/>
							{!isValided && <p className="text-red-500">{errors?.address?.[0]}</p>}
						</div>
					</div>
